            ...
            for (count, bubble) in enumerate(bubble_loader):
                ...
                if count == 0:
                    if transform is not None:
                        ...
                        current = bubble
                elif count >  100 and (count - 1) % 100 == 0:
                    print('new_bubble')
                    if transform is not None:
                        current = transform(bubble)
                    else:
                ...
                wandb.log({'loss_x': error[0], 'loss_y': error[1], 'loss_z': error[2], 'epoch': model.epoch, 'bubble_num': si+1, "loss": loss, 'onesteploss': one_step_loss , 'velocity_centroid_x': vel_pred[ 0], 'velocity_centroid_y': vel_pred[1], 'velocity_centroid_z': vel_pred[ 2], 'true_velocity_centroid_x': vel_true[ 0], 'true_velocity_centroid_y': vel_true[1], 'true_velocity_centroid_z': vel_true[ 2]})
