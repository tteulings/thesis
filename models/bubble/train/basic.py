...
from ..data import Bubble, BubbleDataset
...
# Print header
print("{:-<80}".format(""))
print("Epoch{:2}Train loss{:8}Relative error{:23}Time".format(" ", " ", " "))
print("{:-<80}".format(""))
...
def train(model: BasicModel[Bubble], gather_statistics: bool) -> None:
    count = 0
    train_loss = 0
    rel_error = numpy.array([0, 0, 0], dtype=numpy.float32)
    ...
    while True:
        for bubble in bubble_loader:
            ...
            rel_error += (
                torch.mean(
                    torch.abs(pred - label) / torch.abs(label).clamp_min(eps),
                    0,
                )
                .detach()
                .cpu()
            ...
            if count % args.print_every == 0:
                print(
                    ...
                    f"{f'{rel_error/count}': <37}{default_timer() - timer}"
                )
                sys.stdout.flush()
